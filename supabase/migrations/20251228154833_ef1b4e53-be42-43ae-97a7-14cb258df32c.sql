...
BEGIN
  ...
  -- For category budgets WITHOUT a parent (not sub-budgets), check for overlapping budgets with same category
  IF NEW.category_id IS NOT NULL AND NEW.parent_budget_id IS NULL THEN
    SELECT COUNT(*) INTO overlap_count
    FROM public.budgets
    WHERE user_id = NEW.user_id
      AND category_id = NEW.category_id
      AND parent_budget_id IS NULL  -- Only check against other parent/standalone budgets
